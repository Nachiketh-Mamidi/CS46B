...
public class SimpledokuGrid {
	...
	public boolean isLegal()
	{
		...
		for (int i = 0; i < nCellsPerSide; i ++) {
			for (int j = 0; j < nCellsPerSide; j ++) {
				toCheck.add(this.values[i][j]);
			}
			if (containsNonZeroRepeats(toCheck) == true) {
				return false;
			}
			toCheck.clear();
		}
		...
		for (int i = nCellsPerSide; i > 0; i --) {
			toCheck.add(this.values[i - 1][nCellsPerSide - i]);
		}
		if (containsNonZeroRepeats(toCheck) == true) {
			return false;
		}
	...
	public Evaluation evaluate()
	{
		...
		} else if (this.isLegal() && this.isFull()) {
	...
	{		
		...
		for (int i = 1; i <= this.nCellsPerSide; i ++) {
			SimpledokuGrid next = new SimpledokuGrid(this);
			int[] nextPos = this.getIndicesOfNextEmptyCell();
			next.values[nextPos[0]][nextPos[1]] = i;
			nextGrids.add(next);
		}
		return nextGrids;
	}
